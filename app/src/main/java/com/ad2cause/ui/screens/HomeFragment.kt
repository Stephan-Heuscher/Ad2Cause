...
import com.ad2cause.ads.AdManager
import com.ad2cause.databinding.FragmentHomeBinding
import com.ad2cause.viewmodel.AdViewModel
...
class HomeFragment : Fragment() {
    ...
    private fun observeViewModel() {
        ...
        // Observe UI events
        adViewModel.uiEvent.observe(viewLifecycleOwner) { event ->
            when (event) {
                is AdViewModel.UiEvent.CauseSelected -> {
                    Toast.makeText(
                        requireContext(),
                        event.causeName,
                        Toast.LENGTH_SHORT
    ...
    private fun setupAdCallbacks() {
        adManager.onRewardEarned = { rewardAmount ->
            val cause = causeViewModel.activeCause.value ?: return@let
